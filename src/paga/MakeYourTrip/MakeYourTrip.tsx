...
export default function MakeYourTrip() {
  ...
  const step2Schema = Yup.object({
    ...
    MinPrice: Yup.number().min(0, "Min price must be 0 or more"),
    MaxPrice: Yup.number().min(0, "Max price must be 0 or more"),
    Note: Yup.string(),
  });
  ...
  const handleNext = async () => {
    if (activeStep === 1) {
      ...
      if (Object.keys(validationErrors).length > 0) {
        formikStep1.setTouched({ From: true, to: true });
        return;
      }
    }
      ...
      <div className="container space-y-6">
        <div className="space-y-8 p-5 lg:py-6 lg:px-10 bg-white rounded-2xl">
          ...
          >
            ...
            <p className="text-[#1D1F1F] font-medium text-2xl">
              Make Your Trip
            </p>
          </div>
