...
export default function PopularCard({
  ...
  return (
    <>
      <Link to={`/card-details/${cardData.id}`}>
        <div
          className="border cursor-pointer border-[#EEEEEE] rounded-3xl p-2 pb-4 flex flex-col gap-4 hover:shadow-xl transition-colors duration-300"
        >
          <div className="w-full h-53.75 rounded-2xl overflow-hidden relative">
            <PropertySwiper images={cardData.imge} />
            <div className="size-10 flex items-center justify-center rounded-full absolute end-2 top-2 bg-white z-40">
              <img
                src={icon1}
                alt=""
                width={20}
